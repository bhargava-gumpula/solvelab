...
- Signed-in timer data lives in Firestore (`users/{uid}/…`). IndexedDB is a working copy so the timer stays fast. **Coach, Stats, Train and Learn need a Google account** (`lib/auth/access.ts`, `components/auth/require-account.tsx`); the timer, Algorithms and Settings are open. **Signing out clears this browser**: the database is dropped and every `solvelab.` key goes except the appearance one (`lib/storage/reset.ts`), then the page reloads at `/timer/`. Builds with no Firebase config lock nothing, so the e2e suite and local builds without `.env.local` still work. Train and Learn are still "coming later" pages in 3.1.
