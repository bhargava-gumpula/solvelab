...
**3.1 coach (phases 1–2 so far):** Coach asks for a goal, then points to the next skill test. Twelve tests live at `/coach/tests/<id>/`: cross, F2L, OLL, PLL, cross + F2L, single pair, last pair + OLL, OLL + PLL, cross planning, slow F2L, turning speed and cross + first pair. Each has steps, a goal, progress, and attempts that can be deleted with Undo. Test times never touch timer solves. **Stats → Solve profile** (`/stats/profile/`) rates 15 aspects slow/average/fast against the goal and shows a trend, the next test, and every test's status. Finished tests are shared to train the coach unless the person turns it off (Settings → Your data). Train and Learn show coming-soon pages; Algorithms is a browsable list of sets. Nothing requires Google sign-in; signing in only syncs data.
...
| Storage + backup      | `lib/storage/*` (Dexie schema v4, migrations, repositories, Zod schemas, `legacy.ts` one-time localStorage imports), `lib/export/backup.ts` (format v2), `hooks/use-local-data.ts`, `hooks/use-view-preference.ts`                                                                                                                                                                      |
...
| Coach + tests         | `lib/coach/aspects.ts` (the 15 aspects), `profile.ts` (numbers, tags, next test), `profile-format.ts`, `test-status.ts`, `goals.ts`, `data/milestones/aspect-targets.ts` (goals per level, sources), `data/exercises` (`TEST_ORDER`), `components/tests/*` (test page, timer card, attempts, aspect card), `components/stats/solve-profile.tsx`, `components/coach/coach-dashboard.tsx` |
...
**Saved data keys:** IndexedDB `speedcubing-local` (schema v4 adds `profileSnapshots`; never rename, add migrations with tests). The settings record holds timer options, `panelOffsets`, `appearance` and `view` (Stats range/session, times sort). localStorage holds caches only: `solvelab.appearance.v1` (for the no-flash boot script), `solvelab.panels.v4`, `solvelab.sync.tombstones.v1`, the device-only `solvelab.timerDevice.v1`, and for coach training `solvelab.trainingContributors.v1` (ids this browser shared under) and `solvelab.trainingWithdrawPending.v1`; `solvelab.lessonProgress.v1` is imported into IndexedDB once and removed. Backup format id `speedcubing-local-backup`, version 2 (v1 still imports). **Every user choice must be saved in a synced table or the settings record** (owner rule); signed-in data lives in Firestore at `users/{uid}/{table}/{key}` for each table in `lib/sync/collections.ts`, plus `settings/preferences` and `tombstones`.
...
2. ✅ **Tests and Solve profile** (phase 2, awaiting review; dev log 106–108): (15 aspects with tag, goal, current average; many tests incl. transitions, lookahead, TPS; delete accidental attempts) plus **data contribution** for AI training (on by default, opt-out, signed-out users via Firebase anonymous auth; privacy policy and terms updated).
